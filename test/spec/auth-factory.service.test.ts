...
import { OKTA_AUTH, provideOktaAuth } from "../../lib/src/okta-angular";
  ...
  describe("createOktaAuth", () => {
    describe("auth-js major version compatibility", () => {
      ...
      it("throws when version not match", () => {
        ...
        setupForRoot(oktaAuth);
        expect(() => TestBed.inject(OKTA_AUTH)).toThrow(
          new AuthSdkError(
            `Passed in oktaAuth is not compatible with the SDK, minimum supported okta-auth-js version is 5.3.1.`
          )
        );
      });
    });
    ...
    describe("Okta User Agent tracking", () => {
      it("adds sdk environment to oktaAuth instance", () => {
        ...
        expect(oktaAuth._oktaUserAgent.addEnvironment).toHaveBeenCalledWith(
          "@okta/okta-angular/99.9.9"
        );
      });
