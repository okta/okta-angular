...
> :warning: `okta-angular` 6.0+ supports Angular 12-16. For Angular 7 to 11 please use `okta-angular` 5.x
> :warning: Angular versions older than 16 may not be fully compatible with all dependencies of this library, due to an older Typescript version which does not contain a definition for the `unknown` type. You may be able to workaround this issue by setting `skipLibChecks: true` in your `tsconfig.json` file.
> :note: This library is tested using NgModules architecture. You can import this library in a stand-alone architecture Angular project, using `importProvidersFrom(OktaAuthModule)` if you don't use the `HasAnyGroup` structural directive. 
